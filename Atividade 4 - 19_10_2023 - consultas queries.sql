...
select 	u.name, 
        count(m.fk_sid) as TOTAL -- conta quantas lojas cada user gerencia
from users u
inner join seller s on u.pk_userid = s.pk_userid
inner join manage m on s.pk_userid = m.fk_userid
group by u.name
having total > 1;
--
...
select distinct s.pk_sid, s.name 
from store s
...
-- OrderNumber, Nome, Rua, ContactPhone, City, PostalCode
select * from orders;
select * from address;
select * from deliver_to;

select 	d.fk_orderNumber as Ordem, 
        a.name as Nome, 
        a.streetaddr as Rua, 
...
inner join address as a on fk_addrid = a.pk_addrid
where d.fk_orderNumber = 12992012;
--
